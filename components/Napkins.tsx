        ...
        <div className="mt-auto flex flex-col gap-3 pt-3 border-t border-slate-900/5 dark:border-white/5">
            ...
            <div className="flex justify-between items-center w-full">
                 ...
                 <div className="flex gap-2 justify-end">
                    {!isArchived ? (
                        <>
                             <button onClick={(e) => { e.stopPropagation(); if(window.confirm('В Трекер?')) { onAddTask({ id: Date.now().toString(), content: note.content, column: 'todo', createdAt: Date.now() }); archiveNote(note.id); } }} className="flex items-center gap-1.5 text-[10px] md:text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 px-2 md:px-3 py-1.5 rounded-lg transition-colors border border-emerald-100 dark:border-emerald-800/50" title="В Трекер"><Kanban size={14} /></button>
                             <button onClick={(e) => { e.stopPropagation(); if(window.confirm('В Разбор?')) moveNoteToSandbox(note.id); }} className="flex items-center gap-1.5 text-[10px] md:text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 hover:bg-amber-100 dark:hover:bg-amber-900/40 px-2 md:px-3 py-1.5 rounded-lg transition-colors border border-amber-100 dark:border-amber-800/50" title="В Разбор"><Box size={14} /></button>
    ...
    <div className="flex flex-col h-full max-w-4xl mx-auto p-3 md:p-8 space-y-4 md:space-y-6 relative overflow-y-auto">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1 md:mt-2 text-sm">Свалка для идей. Запиши, чтобы не забыть — разберешь потом.</p>
        </div>
                ...
                {inboxNotes.length > 0 ? (
                    ...
                    <div className="col-span-1 md:col-span-2 py-6">
                        <EmptyState 
                          icon={PenTool} 
                          title="Чистый лист" 
                          description={searchQuery || activeColorFilter || aiFilteredIds || tagQuery ? 'Ничего не найдено по вашему запросу.' : 'Инбокс пуст. Это отличное начало для новых идей.'}
                        />
                    </div>
                )}
            </div>
