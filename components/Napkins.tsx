...
const Napkins: React.FC<Props> = ({ notes, config, addNote, moveNoteToSandbox, moveNoteToInbox, archiveNote, deleteNote, reorderNote, updateNote, onAddTask, onAddJournalEntry, addSketchItem, defaultTab }) => {
  ...
  const handleDump = async () => {
    ...
    addNote(newNote);
    setTitle(''); setCreationColor('white'); setCreationCover(null); setContent('');
    setCreationTags([]);
  ...
  const markdownRenderComponents = {
      ...
      a: ({node, ...props}: any) => <a className="text-indigo-600 dark:text-indigo-400 hover:underline cursor-pointer break-all" target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} {...props} />,
      ...
      code: ({node, inline, className, children, ...props}: any) => {
           return inline 
              ? <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-[10px] font-mono text-pink-600 dark:text-pink-400" {...props}>{children}</code>
                                    ...
                                    <div ref={editorRef} className={`${getNoteColorClass(creationColor)} rounded-3xl transition-all duration-300 shrink-0 relative mb-8 ${isExpanded ? 'shadow-xl z-30' : 'shadow-sm hover:shadow-md'}`}>
                                        ...
                                        {!isExpanded ? (
                                            ...
                                            <div className="flex flex-col animate-in fade-in duration-200 relative z-10">
                                                ...
                                                <div className="flex items-center justify-end px-4 py-3 gap-2 bg-transparent border-t border-slate-100 dark:border-slate-700/50">
                                                    <div className="relative">
                                                    ...
                                                    <button onClick={handleDump} disabled={isProcessing} className="text-xs font-bold uppercase tracking-wider px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl hover:opacity-90 transition-all disabled:opacity-50">Сохранить</button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
