...
// SYNTHESIS CHAMBER - ALCHEMY OF THOUGHT
const SynthesisChamber = ({ 
    ...
    // Stage State: What the user is currently editing
    const [activeModule, setActiveModule] = useState<'kanban' | 'rituals' | 'journal' | null>(null);
    
    // Staged Data: What the user has committed to creating
    const [stagedTask, setStagedTask] = useState<string | null>(null);
    const [stagedHabit, setStagedHabit] = useState<string | null>(null);
    const [stagedInsight, setStagedInsight] = useState<boolean>(false);
    
    // Input buffers
    const [taskInput, setTaskInput] = useState(note.title || '');
    const [habitInput, setHabitInput] = useState(note.title || 'Новый ритуал');

    // Derived Metrics for "Bio-Balance"
    const density = (stagedTask ? 1 : 0) + (stagedHabit ? 1 : 0) + (stagedInsight ? 1 : 0);
    ...
    return createPortal(
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 overflow-hidden">
            ...
            <motion.div 
                ...
                className="relative w-full max-w-[900px] h-[700px] flex flex-col items-center justify-center"
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    ...
                    <motion.div 
                        animate={{ rotate: 360 }}
                        transition={{ duration: 60 - (density * 15), repeat: Infinity, ease: "linear" }}
                        className="w-[400px] h-[400px] rounded-full border border-dashed border-slate-500/20"
                    />
                    {/* Ring 2 - Medium */}
                    <motion.div 
                        animate={{ rotate: -360 }}
                        transition={{ duration: 40 - (density * 10), repeat: Infinity, ease: "linear" }}
                        className="absolute w-[280px] h-[280px] rounded-full border border-dotted border-indigo-500/20"
                    />
                    {/* Ring 3 - Fast (Appears on density) */}
                    {density > 0 && (
                        <motion.div 
                            initial={{ scale: 0 }}
                            animate={{ scale: 1, rotate: 360 }}
                            transition={{ duration: 20 - (density * 5), repeat: Infinity, ease: "linear" }}
                            className="absolute w-[180px] h-[180px] rounded-full border-2 border-white/10 border-t-white/50"
                        />
                    )}
                    ...
                    <motion.div 
                        ...
                        animate={{ 
                            scale: 1 + (density * 0.1),
                            filter: `blur(${Math.max(0, 10 - density * 4)}px)`
                        }}
                    >
                        <div className="w-24 h-24 bg-white/5 backdrop-blur-md rounded-xl rotate-45 border border-white/20 flex items-center justify-center shadow-[0_0_50px_rgba(255,255,255,0.1)]">
                            <Box size={32} className={`text-white transition-all ${density > 0 ? 'opacity-100' : 'opacity-50'}`} strokeWidth={1} />
                        </div>
                    </motion.div>
                </div>

                {/* --- STRUCTURAL BRIDGES (CONNECTORS) --- */}
                <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
                    ...
                    <motion.line 
                        ...
                        stroke={stagedTask ? "#10b981" : "url(#beamGrad)"}
                        strokeWidth={stagedTask ? 2 : 1}
                        strokeDasharray={stagedTask ? "none" : "4 4"}
                        initial={{ pathLength: 0 }}
                        animate={{ pathLength: 1, opacity: activeModule === 'kanban' || stagedTask ? 1 : 0.2 }}
                    />
                    ...
                    <motion.path 
                        ...
                        fill="none"
                        stroke={stagedHabit ? "#f43f5e" : "url(#beamGrad)"}
                        strokeWidth={stagedHabit ? 2 : 1}
                        strokeDasharray={stagedHabit ? "none" : "4 4"}
                        initial={{ pathLength: 0 }}
                        animate={{ pathLength: 1, opacity: activeModule === 'rituals' || stagedHabit ? 1 : 0.2 }}
                    />

                    {/* Bottom Beam (Journal) */}
                    <motion.line 
                        x1="50%" y1="50%" x2="50%" y2="80%" 
                        stroke={stagedInsight ? "#06b6d4" : "url(#beamGrad)"}
                        strokeWidth={stagedInsight ? 2 : 1}
                        strokeDasharray={stagedInsight ? "none" : "4 4"}
                        initial={{ pathLength: 0 }}
                        animate={{ pathLength: 1, opacity: activeModule === 'journal' || stagedInsight ? 1 : 0.2 }}
                    />
                </svg>
                ...
                {/* RIGHT: KANBAN */}
                <div className="absolute right-[10%] top-1/2 -translate-y-1/2 flex flex-col items-center gap-4 z-20">
                    {activeModule === 'kanban' ? (
                        <motion.div 
                            initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                            className="bg-black/40 backdrop-blur-md border border-emerald-500/50 p-4 rounded-xl w-64 shadow-[0_0_30px_rgba(16,185,129,0.2)]"
                        >
                            <div className="text-[10px] font-mono font-bold text-emerald-500 mb-2 uppercase tracking-widest">ИНТЕГРАЦИЯ В СПРИНТ</div>
                            <input 
                                value={taskInput}
                                onChange={e => setTaskInput(e.target.value)}
                                className="w-full bg-transparent border-b border-emerald-500/30 text-white text-sm py-1 outline-none font-sans mb-3"
                                placeholder="Определить цель..."
                                autoFocus
                            />
                            <div className="flex gap-2">
                                <button onClick={() => { setStagedTask(taskInput); setActiveModule(null); }} className="flex-1 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 text-[10px] py-1.5 rounded uppercase font-bold transition-colors">Подтвердить</button>
                                <button onClick={() => setActiveModule(null)} className="text-slate-500 hover:text-slate-300 text-[10px] uppercase">Отмена</button>
                            </div>
                        </motion.div>
                    ) : (
                        <button 
                            onClick={() => stagedTask ? setStagedTask(null) : setActiveModule('kanban')}
                            className={`w-16 h-16 rounded-2xl flex items-center justify-center border transition-all duration-500 group ${stagedTask ? 'bg-emerald-500/10 border-emerald-500 text-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.4)]' : 'bg-white/5 border-white/10 text-slate-500 hover:border-emerald-500/50 hover:text-emerald-400'}`}
                        >
                            {stagedTask ? <Check size={24} /> : <Kanban size={24} strokeWidth={1.5} />}
                        </button>
                    )}
                    <span className="text-[9px] font-mono uppercase tracking-widest text-slate-500">ДЕЙСТВИЕ</span>
                </div>

                {/* LEFT: RITUALS */}
                <div className="absolute left-[10%] top-1/2 -translate-y-1/2 flex flex-col items-center gap-4 z-20">
                    {activeModule === 'rituals' ? (
                        <motion.div 
                            initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                            className="bg-black/40 backdrop-blur-md border border-rose-500/50 p-4 rounded-xl w-64 shadow-[0_0_30px_rgba(244,63,94,0.2)]"
                        >
                            <div className="text-[10px] font-mono font-bold text-rose-500 mb-2 uppercase tracking-widest">ЗАКРЕПИТЬ ПОВЕДЕНИЕМ</div>
                            <input 
                                value={habitInput}
                                onChange={e => setHabitInput(e.target.value)}
                                className="w-full bg-transparent border-b border-rose-500/30 text-white text-sm py-1 outline-none font-sans mb-3"
                                placeholder="Название привычки..."
                                autoFocus
                            />
                            <div className="flex gap-2">
                                <button onClick={() => { setStagedHabit(habitInput); setActiveModule(null); }} className="flex-1 bg-rose-500/20 hover:bg-rose-500/30 text-rose-400 text-[10px] py-1.5 rounded uppercase font-bold transition-colors">Подтвердить</button>
                                <button onClick={() => setActiveModule(null)} className="text-slate-500 hover:text-slate-300 text-[10px] uppercase">Отмена</button>
                            </div>
                        </motion.div>
                    ) : (
                        <button 
                            onClick={() => stagedHabit ? setStagedHabit(null) : setActiveModule('rituals')}
                            className={`w-16 h-16 rounded-full flex items-center justify-center border transition-all duration-500 group ${stagedHabit ? 'bg-rose-500/10 border-rose-500 text-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.4)]' : 'bg-white/5 border-white/10 text-slate-500 hover:border-rose-500/50 hover:text-rose-400'}`}
                        >
                            {stagedHabit ? <Check size={24} /> : <Flame size={24} strokeWidth={1.5} />}
                        </button>
                    )}
                    <span className="text-[9px] font-mono uppercase tracking-widest text-slate-500">СИСТЕМА</span>
                </div>

                {/* BOTTOM: JOURNAL */}
                <div className="absolute bottom-[15%] left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 z-20">
                    {activeModule === 'journal' ? (
                        <motion.div 
                            initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
                            className="bg-black/40 backdrop-blur-md border border-cyan-500/50 p-4 rounded-xl w-64 text-center shadow-[0_0_30px_rgba(6,182,212,0.2)]"
                        >
                            <div className="text-[10px] font-mono font-bold text-cyan-500 mb-3 uppercase tracking-widest">ОСОЗНАТЬ ИНСАЙТ</div>
                            <p className="text-xs text-slate-300 mb-4 font-serif italic">"Зафиксировать эту мысль как ключевой инсайт в дневнике?"</p>
                            <div className="flex gap-2">
                                <button onClick={() => { setStagedInsight(true); setActiveModule(null); }} className="flex-1 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 text-[10px] py-1.5 rounded uppercase font-bold transition-colors">Да, это важно</button>
                                <button onClick={() => setActiveModule(null)} className="text-slate-500 hover:text-slate-300 text-[10px] uppercase">Нет</button>
                            </div>
                        </motion.div>
                    ) : (
                        <button 
                            onClick={() => stagedInsight ? setStagedInsight(false) : setActiveModule('journal')}
                            className={`w-16 h-16 rounded-xl flex items-center justify-center border transition-all duration-500 group rotate-45 ${stagedInsight ? 'bg-cyan-500/10 border-cyan-500 text-cyan-500 shadow-[0_0_20px_rgba(6,182,212,0.4)]' : 'bg-white/5 border-white/10 text-slate-500 hover:border-cyan-500/50 hover:text-cyan-400'}`}
                        >
                            <Book size={24} strokeWidth={1.5} className="-rotate-45" />
                        </button>
                    )}
                    <span className="text-[9px] font-mono uppercase tracking-widest text-slate-500">СМЫСЛ</span>
                </div>

                {/* --- IMPACT METER (HUD) --- */}
                <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end pointer-events-none">
                    <div className="flex flex-col gap-1">
                        <div className="text-[9px] font-mono text-slate-500 uppercase tracking-widest mb-1">BIO-BALANCE PROJECTION</div>
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                                <div className={`h-1 w-12 rounded-full transition-colors ${stagedTask ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-slate-800'}`} />
                                <span className={`text-[10px] font-bold ${stagedTask ? 'text-emerald-500' : 'text-slate-700'}`}>GROWTH</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className={`h-1 w-12 rounded-full transition-colors ${stagedHabit ? 'bg-rose-500 shadow-[0_0_8px_#f43f5e]' : 'bg-slate-800'}`} />
                                <span className={`text-[10px] font-bold ${stagedHabit ? 'text-rose-500' : 'text-slate-700'}`}>DISCIPLINE</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className={`h-1 w-12 rounded-full transition-colors ${stagedInsight ? 'bg-cyan-500 shadow-[0_0_8px_#06b6d4]' : 'bg-slate-800'}`} />
                                <span className={`text-[10px] font-bold ${stagedInsight ? 'text-cyan-500' : 'text-slate-700'}`}>WISDOM</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        disabled={density === 0}
                        className={`pointer-events-auto px-8 py-4 bg-white text-black font-mono text-xs font-bold uppercase tracking-[0.2em] hover:bg-slate-200 transition-all clip-path-polygon disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:shadow-[0_0_50px_rgba(255,255,255,0.3)]`}
                ...
                {/* Top Hub Link (Optional) */}
                <div className="absolute top-[15%] left-1/2 -translate-x-1/2 pointer-events-auto">
                     <button onClick={() => onMoveToSandbox(note.id)} className="text-slate-600 hover:text-amber-500 transition-colors flex flex-col items-center gap-2 group">
                        <Box size={20} />
                        <span className="text-[9px] font-mono uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">В ХАБ</span>
                     </button>
                </div>
...
const Napkins: React.FC<Props> = ({ notes, config, addNote, moveNoteToSandbox, moveNoteToInbox, archiveNote, deleteNote, reorderNote, updateNote, onAddTask, onAddJournalEntry, addSketchItem, addHabit, defaultTab, initialNoteId, onClearInitialNote, journalEntries }) => {
  ...
  // --- SYNTHESIS CHAMBER (REPLACES HERO JOURNEY) ---
  const SynthesisChamber = ({ 
      note, 
      onClose, 
      onAddTask,
