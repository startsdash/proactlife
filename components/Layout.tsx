...
import { StickyNote, Box, Dumbbell, Kanban as KanbanIcon, Settings, Cloud, CloudOff, RefreshCw, CheckCircle2, AlertCircle, History, Book, GraduationCap, ChevronLeft, ChevronRight, PanelLeftClose, PanelLeftOpen, UserCog, Shield, Menu, Moon, Sun, Flame } from 'lucide-react';
...
const Layout: React.FC<Props> = ({ currentModule, setModule, children, syncStatus, onConnectDrive, isDriveConnected, isOwner }) => {
  ...
  // Initialize state based on screen size and theme preference
  useEffect(() => {
    const handleResize = () => {
  ...
  const navItems = [
    ...
    { id: Module.RITUALS, icon: Flame, label: 'Ритуалы' }, // NEW
  ...
  return (
    <div className="flex h-[100dvh] bg-[#f8fafc] dark:bg-slate-950 text-slate-800 dark:text-slate-200 font-sans overflow-hidden transition-colors duration-300">
      ...
      {isMobile && !isExpanded && (
          <button 
            ...
            className="fixed bottom-6 left-4 z-40 p-3 bg-slate-900 dark:bg-slate-800 text-white rounded-full shadow-lg shadow-slate-300 dark:shadow-slate-900/50 md:hidden animate-in zoom-in-95 duration-200 hover:bg-slate-800 active:scale-95"
            title="Меню"
          >
            <Menu size={24} />
          </button>
      ...
      <aside 
        className={`
            bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between shrink-0 z-50
      ...
      >
        <div>
          {/* HEADER / LOGO */}
          <div className={`h-16 md:h-20 flex items-center border-b border-slate-100 dark:border-slate-800 transition-all duration-300 ${isExpanded ? 'px-6 justify-between' : 'justify-center px-0'}`}>
             <div className="flex items-center overflow-hidden whitespace-nowrap">
                ...
                <div className={`ml-3 transition-all duration-300 origin-left ${isExpanded ? 'opacity-100 w-auto' : 'opacity-0 w-0 hidden'}`}>
                    ...
                    <div className="text-[10px] text-slate-400 dark:text-slate-500 font-mono tracking-wider mt-0.5">PRO v2.1</div>
                </div>
             </div>
              ...
              <button 
                ...
                className={`
                    ...
                    ${currentModule === item.id 
                        ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-indigo-900/20' 
        ...
        <div className="p-3 space-y-2 border-t border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/20">
             ...
             <button 
                ...
                className={`
                    ...
                    text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-white dark:hover:bg-slate-800
                    ${isExpanded ? 'justify-start' : 'justify-center'}
                `}
                title={!isExpanded ? "Тема" : undefined}
             >
                <div className="relative shrink-0">
                 ...
                 <div className={`ml-3 flex flex-col items-start overflow-hidden transition-all duration-300 ${isExpanded ? 'opacity-100 w-auto' : 'opacity-0 w-0 h-0'}`}>
                    <span className="text-sm font-medium whitespace-nowrap">{isDarkMode ? 'Темная тема' : 'Светлая тема'}</span>
                </div>
             </button>
             ...
             <button 
                ...
                className={`
                    ...
                    ${!isDriveConnected ? 'hover:bg-indigo-50 dark:hover:bg-indigo-900/20 cursor-pointer' : 'cursor-default'}
                    ${isExpanded ? 'justify-start' : 'justify-center'}
                `} 
                title={!isExpanded ? "Облако" : undefined}
             >
                <div className="relative shrink-0">
                    <Cloud size={20} className={isDriveConnected ? 'text-indigo-500' : 'text-slate-400 dark:text-slate-500'} />
                    <div className="absolute -bottom-1 -right-1 bg-white dark:bg-slate-900 rounded-full ring-2 ring-white dark:ring-slate-900">{getSyncIcon()}</div>
                </div>
             ...
             <button 
                ...
                className={`
                    w-full flex items-center p-3 rounded-xl transition-all duration-200
                    ${currentModule === Module.USER_SETTINGS ? 'bg-slate-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}
                    ${isExpanded ? 'justify-start' : 'justify-center'}
                `}
             ...
             {isOwner && (
               <button 
                ...
                className={`
                    w-full flex items-center p-3 rounded-xl transition-all duration-200
                    ${currentModule === Module.SETTINGS ? 'bg-slate-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}
                    ${isExpanded ? 'justify-start' : 'justify-center'}
                `}
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 flex flex-col w-full relative overflow-x-hidden overflow-y-auto bg-[#f8fafc] dark:bg-slate-950">
        {children}
      </main>
    </div>
  );
