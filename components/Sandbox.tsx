...
const Sandbox: React.FC<Props> = ({ notes, config, onProcessNote, onAddTask, onAddFlashcard, deleteNote }) => {
  ...
  const handleManualArchive = () => {
      if (selectedNoteId) {
          if (window.confirm("Перенести заметку в Зал славы?")) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#f8fafc] dark:bg-[#0f172a]">
        <header className={`p-4 md:p-8 pb-0 shrink-0 ${selectedNoteId ? 'hidden md:block' : 'block'}`}>
            <h1 className="text-2xl md:text-3xl font-light text-slate-800 dark:text-slate-200 tracking-tight">Менторы</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm">Готовый план действий от ИИ-ментора</p>
        </header>
