...
const Kanban: React.FC<Props> = ({ tasks, journalEntries, config, updateTask, deleteTask, reorderTask, archiveTask, onReflectInJournal, initialTaskId, onClearInitialTask }) => {
                            ...
                            <div className="flex gap-2 items-center justify-end">
                               ...
                               {col.id === 'done' && (
                                    <button 
                                        onClick={(e) => { 
                                            e.stopPropagation(); 
                                            if(window.confirm('Перенести задачу в Базу?')) archiveTask(task.id); 
                                        }} 
  ...
  return (
    <div className="h-full p-3 md:p-8 flex flex-col overflow-hidden relative">
      <header className="mb-4 shrink-0"><h1 className="text-2xl md:text-3xl font-light text-slate-800 dark:text-slate-200 tracking-tight">Трекер <span className="text-emerald-500 text-sm md:text-lg">/ Задачи</span></h1><p className="text-slate-500 dark:text-slate-400 mt-2 text-sm">Канбан-доска. Двигайся от «сделать» к «готово».</p></header>
