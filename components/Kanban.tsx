    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button 
                ...
                className={`w-full p-2.5 rounded-lg border flex items-center justify-between transition-all outline-none bg-transparent hover:bg-black/5 dark:hover:bg-white/5 border-slate-200 dark:border-slate-700`}
            >
                        ...
                        <>
                            <div className="flex -space-x-1.5 shrink-0">
                                {selected.map(s => {
                                    ...
                                    return sp ? <div key={s} className={`w-3.5 h-3.5 rounded-full border-[1.5px] bg-transparent ${sp.text.replace('text-', 'border-')}`}></div> : null;
                                })}
                            </div>
                            <span className="text-xs font-medium text-[#2F3437] dark:text-slate-200 truncate">
                                {selected.map(id => SPHERES.find(s => s.id === id)?.label).join(', ')}
                            </span>
                        </>
                    ) : (
                        <span className="text-xs text-[#6B6E70]">Сферы...</span>
                    )}
                </div>
                <ChevronDown size={14} className={`text-[#6B6E70] transition-transform ${isOpen ? 'rotate-180' : ''}`} />
            </button>
...
const CollapsibleSection: React.FC<{
  ...
  return (
    <div className={`${isCard ? 'bg-slate-50/50 dark:bg-slate-800/30 mb-2' : 'bg-slate-50/50 dark:bg-slate-800/30 mb-3'} rounded-xl border border-slate-100 dark:border-slate-700/50 overflow-hidden`}>
      <div 
        onClick={(e) => { e.stopPropagation(); setIsOpen(!isOpen); }} 
        className={`w-full flex items-center justify-between ${isCard ? 'p-2' : 'p-3'} cursor-pointer hover:bg-slate-100/50 dark:hover:bg-slate-700/30 transition-colors group/header`}
      >
      ...
      {isOpen && (
        <div className={`${isCard ? 'px-2 pb-2' : 'px-3 pb-3'} pt-0 animate-in slide-in-from-top-1 duration-200`}>
        ...
        {activeModal && (
            <div className="fixed inset-0 z-[100] bg-slate-900/20 dark:bg-black/60 backdrop-blur-2xl flex items-center justify-center p-4" onClick={handleCloseModal}>
                <motion.div 
                    ...
                    className="w-full max-w-lg bg-[rgba(255,255,255,0.72)] dark:bg-[rgba(30,41,59,0.72)] backdrop-blur-[40px] saturate-150 border border-black/5 dark:border-white/10 rounded-[32px] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] p-8 md:p-10 flex flex-col max-h-[90vh] relative overflow-hidden"
                    onClick={(e) => e.stopPropagation()}
                >
                    {/* NO COVER IMAGE - TECHNICAL STYLE */}
                    ...
                    <div className="flex justify-between items-start mb-6 shrink-0 border-b border-black/5 dark:border-white/5 pb-4">
                        <div className="flex flex-col gap-2 pr-4 w-full">
                            {activeModal.type === 'details' ? (
                                <input 
                                    ...
                                    className={`text-2xl font-sans font-semibold text-[#2F3437] dark:text-[#EBEBEB] leading-[1.3] bg-transparent border-none outline-none placeholder:text-slate-300 dark:placeholder:text-slate-600 w-full p-0 m-0 ${isEditingTask ? 'border-b border-slate-900/10 dark:border-white/10 pb-1' : ''}`} 
                                    readOnly={!isEditingTask}
                                />
                            ) : (
                                <h3 className="text-2xl font-sans font-semibold text-[#2F3437] dark:text-[#EBEBEB] leading-[1.3]">
                            ...
                            {/* METADATA ROW (View Only) */}
                            {activeModal.type === 'details' && !isEditingTask && (
                                <div className="flex items-center gap-4 text-[10px] font-mono text-slate-400 uppercase tracking-wider mt-1">
                                    <div className="flex items-center gap-1.5">
                                        <div className={`w-2.5 h-2.5 rounded-full border-[1.5px] ${getTaskForModal()?.column === 'done' ? 'border-emerald-500' : getTaskForModal()?.column === 'doing' ? 'border-indigo-500' : 'border-slate-400'} bg-transparent`} />
                                        <span>{getTaskForModal()?.column === 'done' ? 'DONE' : getTaskForModal()?.column === 'doing' ? 'DOING' : 'TODO'}</span>
                                    </div>
                                    {getTaskForModal()?.spheres && getTaskForModal()!.spheres!.length > 0 && (
                                        <>
                                            <div className="w-px h-3 bg-slate-300 dark:bg-slate-600"></div>
                                            <div className="flex -space-x-1">
                                                {getTaskForModal()!.spheres!.map(s => {
                                                    const sp = SPHERES.find(x => x.id === s);
                                                    return sp ? <div key={s} className={`w-2.5 h-2.5 rounded-full border-[1.5px] bg-transparent ${sp.text.replace('text-', 'border-')}`} /> : null;
                                                })}
                                            </div>
                                        </>
                                    )}
                                    ...
                                    <div className="flex items-center gap-1">
                                        <Calendar size={10} />
                                        {new Date(getTaskForModal()?.createdAt || Date.now()).toLocaleDateString()}
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex items-center shrink-0 gap-1">
                            {activeModal.type === 'details' && !isEditingTask && (
                                (() => {
                                    ...
                                    if (task && task.column !== 'done') {
                                        return (
                                            <>
                                                <Tooltip content="Редактировать"><button onClick={() => setIsEditingTask(true)} className="p-2 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg transition-colors"><Edit3 size={18} strokeWidth={1.5} /></button></Tooltip>
                                                <Tooltip content="Удалить"><button onClick={() => { if(window.confirm('Удалить задачу?')) { deleteTask(task.id); handleCloseModal(); } }} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-lg transition-colors border border-transparent hover:border-red-200 dark:hover:border-red-900/30"><Trash2 size={18} strokeWidth={1.5} /></button></Tooltip>
                                            </>
                                        );
                                    }
                                    return null;
                            ...
                            <button onClick={handleCloseModal} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg transition-colors ml-1"><X size={20} strokeWidth={1.5} /></button>
                        </div>
                    </div>

                    <div className="flex-1 overflow-y-auto custom-scrollbar-ghost pr-1 -mr-2">
                                ...
                                <div className="space-y-6">
                                    {isEditingTask ? (
                                        <div className="flex flex-col animate-in fade-in duration-200 relative z-10">
                                            <div className="flex items-center justify-between mb-2 gap-2">
                                                <div className="flex items-center gap-1 pb-1 overflow-x-auto scrollbar-none flex-1 mask-fade-right">
                                                    <Tooltip content="Отменить"><button onMouseDown={(e) => { e.preventDefault(); execEditUndo(); }} disabled={editHistoryIndex <= 0} className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg text-slate-500 dark:text-slate-400 transition-colors disabled:opacity-30"><RotateCcw size={16} /></button></Tooltip>
                                                    <Tooltip content="Повторить"><button onMouseDown={(e) => { e.preventDefault(); execEditRedo(); }} disabled={editHistoryIndex >= editHistory.length - 1} className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg text-slate-500 dark:text-slate-400 transition-colors disabled:opacity-30"><RotateCw size={16} /></button></Tooltip>
                                                    <div className="w-px h-4 bg-slate-300 dark:bg-slate-600 mx-1 shrink-0"></div>
                                                    <Tooltip content="Жирный"><button onMouseDown={(e) => { e.preventDefault(); execEditCmd('bold'); }} className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg text-slate-500 dark:text-slate-400 transition-colors"><Bold size={16} /></button></Tooltip>
                                                    <Tooltip content="Курсив"><button onMouseDown={(e) => { e.preventDefault(); execEditCmd('italic'); }} className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg text-slate-500 dark:text-slate-400 transition-colors"><Italic size={16} /></button></Tooltip>
                                                    <div className="w-px h-4 bg-slate-300 dark:bg-slate-600 mx-1 shrink-0"></div>
                                            ...
                                            <div 
                                                ...
                                                className="w-full min-h-[160px] bg-transparent rounded-xl p-2 text-base text-slate-800 dark:text-slate-200 outline-none overflow-y-auto font-sans leading-[1.6] [&_h1]:text-2xl [&_h1]:font-bold [&_h1]:mb-2 [&_h2]:text-xl [&_h2]:font-bold [&_h2]:mb-2 [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_li]:mb-1 cursor-text custom-scrollbar-ghost border border-transparent focus:border-b-slate-900/10 dark:focus:border-b-white/10"
                                                data-placeholder="Описание задачи..." 
                                            />
                                            <div className="mt-4 pt-4 border-t border-black/5 dark:border-white/5">
                                                <label className="text-[10px] font-bold text-slate-400 uppercase mb-2 block font-mono">Сферы</label>
                                                
                                                {/* GHOST SPHERE SELECTOR */}
                                                    ...
                                                    {SPHERES.map(s => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    const current = task.spheres || [];
                                                                    const newSpheres = current.includes(s.id) ? current.filter(x => x !== s.id) : [...current, s.id];
                                                                    updateTask({...task, spheres: newSpheres});
                                                                }}
                                                                className={`w-6 h-6 rounded-full border-[1.5px] transition-all ${isSelected ? s.text.replace('text-', 'border-') : 'border-slate-300 dark:border-slate-600 hover:border-slate-400'}`}
                                                                title={s.label}
                                                            />
                                                        );
                                                    })}
                                            ...
                                            <div className="flex flex-col-reverse md:flex-row justify-end items-stretch md:items-center gap-4 pt-6 border-t border-black/5 dark:border-white/5 mt-4">
                                                <button onClick={() => setIsEditingTask(false)} className="px-6 py-2 text-[10px] uppercase font-mono tracking-wider text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 transition-colors">Отмена</button>
                                                <button onClick={handleSaveTaskContent} className="px-6 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-[10px] uppercase font-mono tracking-wider font-bold text-slate-800 dark:text-slate-200 hover:bg-black/5 dark:hover:bg-white/5 transition-colors">Сохранить</button>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="group relative pr-1">
                                            ...
                                            <div className="text-[#2F3437] dark:text-slate-300 text-base font-normal leading-[1.6] font-sans mb-4">
                                                <ReactMarkdown components={markdownComponents}>{formatForDisplay(applyTypography(task.content))}</ReactMarkdown>
                                            </div>
                                                    ...
                                                    <div className="space-y-1">
                                                        {task.subtasks?.map(s => (
                                                            <div key={s.id} className="group flex items-start gap-3 p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg transition-all duration-200 hover:translate-x-0.5 cursor-pointer relative" draggable={!isDone} onDragStart={!isDone ? (e) => handleSubtaskDragStart(e, s.id, task.id) : undefined} onDragOver={handleDragOver} onDrop={!isDone ? (e) => handleSubtaskDrop(e, s.id, task) : undefined} onClick={() => !isDone && handleToggleSubtask(s.id)}>
                                                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 transition-all duration-300 mt-0.5 ${s.isCompleted ? 'bg-indigo-500 border-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.5)]' : 'border-slate-300 dark:border-slate-600 group-hover:border-indigo-400 bg-transparent'}`}>{s.isCompleted && <Check size={10} className="text-white" strokeWidth={3} />}</div>
                                                                <span className={`text-sm flex-1 break-words leading-relaxed transition-all duration-300 ${s.isCompleted ? "text-slate-400 line-through opacity-50" : "text-[#2F3437] dark:text-slate-200"}`}>{s.text}</span>
                                                        ...
                                                        {!isDone && (<div className="flex gap-2 mt-3 pl-1"><input type="text" className="flex-1 bg-transparent border-b border-slate-200 dark:border-slate-700 py-1 text-sm outline-none text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:border-indigo-400 transition-colors" placeholder="Новый пункт..." value={newSubtaskText} onChange={(e) => setNewSubtaskText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddSubtask()} /><button onClick={handleAddSubtask} disabled={!newSubtaskText.trim()} className="text-indigo-600 dark:text-indigo-400 hover:bg-black/5 dark:hover:bg-white/5 p-1.5 rounded-lg disabled:opacity-50 transition-colors"><Plus size={18}/></button></div>)}
                                                    </div>
                                                </CollapsibleSection>
                                            )}

                                            {task.activeChallenge && (
                                                <CollapsibleSection title={task.isChallengeCompleted ? "Финальный челлендж" : "Активный челлендж"} icon={task.isChallengeCompleted ? <div className="w-2 h-2 rounded-full border border-emerald-500" /> : <div className="w-2 h-2 rounded-full border border-indigo-500 animate-pulse" />} actions={!isDone ? (<div className="flex items-center gap-3"><Tooltip content={task.isChallengeCompleted ? "Вернуть в активные" : "Завершить челлендж"}><button onClick={(e) => { e.stopPropagation(); toggleChallengeComplete(); }} className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${task.isChallengeCompleted ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-transparent border-slate-300 dark:border-slate-600 hover:border-emerald-500 text-transparent'}`}><Check size={10} strokeWidth={3} /></button></Tooltip><div className="w-px h-3 bg-slate-300 dark:bg-slate-600"></div><Tooltip content="Удалить челлендж"><button onClick={(e) => deleteActiveChallenge(e)} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={14} /></button></Tooltip></div>) : null}>
                                            ...
                                            {((task.challengeHistory && task.challengeHistory.length > 0) || (task.consultationHistory && task.consultationHistory.length > 0)) && (
                                                <CollapsibleSection title="История" icon={<History size={14}/>}>
                                                    <div className="space-y-4">
                                                        {task.challengeHistory?.map((h, i) => (<div key={`ch-${i}`} className="text-sm bg-black/5 dark:bg-white/5 p-4 rounded-xl relative group"><div className="text-[9px] font-bold text-slate-400 mb-2 uppercase tracking-wider flex items-center gap-1"><Zap size={10}/> Архивный челлендж</div><div className="text-[#2F3437] dark:text-slate-300 font-sans text-sm leading-relaxed"><StaticChallengeRenderer content={h} mode="history" /></div>{!isDone && <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"><Tooltip content="Удалить"><button onClick={() => deleteChallengeFromHistory(i)} className="text-slate-300 hover:text-red-500 p-1"><Trash2 size={12}/></button></Tooltip></div>}</div>))}
                                                        {task.consultationHistory?.map((h, i) => (<div key={`cons-${i}`} className="text-sm bg-violet-50/50 dark:bg-violet-900/20 p-4 rounded-xl relative group"><div className="text-[9px] font-bold text-violet-400 mb-2 uppercase tracking-wider flex items-center gap-1"><Bot size={10}/> Консультация</div><div className="text-[#2F3437] dark:text-slate-300 font-sans text-sm leading-relaxed"><ReactMarkdown components={markdownComponents}>{formatForDisplay(applyTypography(h))}</ReactMarkdown></div>{!isDone && <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"><Tooltip content="Удалить"><button onClick={() => deleteConsultation(i)} className="text-slate-300 hover:text-red-500 p-1"><Trash2 size={12}/></button></Tooltip></div>}</div>))}
                                                    </div>
                                                </CollapsibleSection>
                                            )}
                                        </div>
                    ...
                    {activeModal.type === 'stuck' && aiResponse && (<div className="mt-6 flex justify-end gap-2 pt-4 border-t border-black/5 dark:border-white/5 pr-1"><button onClick={saveTherapyResponse} className="px-6 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-[10px] uppercase font-mono tracking-wider font-bold text-slate-800 dark:text-slate-200 hover:bg-black/5 dark:hover:bg-white/5 transition-colors"><Save size={14} className="inline mr-2" /> Сохранить в историю</button></div>)}
                    {activeModal.type === 'challenge' && draftChallenge && (<div className="mt-6 flex justify-end gap-2 shrink-0 pt-4 border-t border-black/5 dark:border-white/5 pr-1"><button onClick={acceptDraftChallenge} className="px-6 py-2 border border-indigo-300 dark:border-indigo-600 rounded-lg text-[10px] uppercase font-mono tracking-wider font-bold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors"><Rocket size={14} className="inline mr-2" /> Принять вызов</button></div>)}
                </motion.div>
            </div>
        )}
      </AnimatePresence>
