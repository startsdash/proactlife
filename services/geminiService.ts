...
export interface NoteActionClassification {
    action: 'task' | 'habit' | 'journal' | 'sandbox';
    reason: string;
}

export const classifyNoteAction = async (noteContent: string, config: AppConfig): Promise<NoteActionClassification> => {
    const ai = getAiClient();
    // Use a lightweight model for classification if available, otherwise default
    const model = DEFAULT_MODEL;
    
    const prompt = `Analyze the following note and decide the best action to take:
    1. 'task': If it's a specific, actionable item or todo.
    2. 'habit': If it describes a repetitive behavior, routine, or ritual.
    3. 'journal': If it's a reflection, feeling, or memory.
    4. 'sandbox': If it's a complex idea, concept, or project needing development.
    
    Note: "${noteContent}"
    
    Return ONLY JSON: { "action": "task" | "habit" | "journal" | "sandbox", "reason": "Short explanation in Russian" }`;
    ...
    try {
        ...
        if (isGemmaModel(model)) {
             ...
             response = await ai.models.generateContent({
                ...
                config: {
                    systemInstruction: "You are an expert productivity assistant. Classify the note.",
                    responseMimeType: "application/json",
                    responseSchema: {
                        type: Type.OBJECT,
                        properties: {
                            action: { type: Type.STRING, enum: ['task', 'habit', 'journal', 'sandbox'] },
        ...
        return parseJSON<NoteActionClassification>(response.text, { action: 'sandbox', reason: 'Требует анализа' });
    } catch (e) {
        console.error("Classification Error", e);
        return { action: 'sandbox', reason: 'Ошибка анализа' };
    }
}
