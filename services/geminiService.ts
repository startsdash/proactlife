...
export interface HeroJourneyAnalysis {
    title: string;
    essence: string;
    archetype: 'task' | 'habit' | 'insight';
    reasoning: string;
}

export const analyzeHeroJourney = async (content: string, config: AppConfig): Promise<HeroJourneyAnalysis | null> => {
    const model = DEFAULT_MODEL; // Or any specific model if needed
    const ai = getAiClient();
    
    const systemInstruction = `
    You are the "Oracle of the Path". You help the user transform chaotic thoughts (Notes) into concrete steps of their Hero's Journey.
    
    Analyze the provided Note Content.
    1. Distill it into a short, inspiring Title (3-5 words).
    2. Extract the "Essence" (1 sentence summary).
    3. Determine the Archetype (Type of Action):
       - 'task': If it's a one-off action, project, or something to be done.
       - 'habit': If it's a recurring behavior, lifestyle change, or system.
       - 'insight': If it's a reflection, idea, or feeling that needs to be journaled.
    ...
    try {
        let response;
        if (isGemmaModel(model)) {
             const gemmaPrompt = `${systemInstruction}\n\n[NOTE CONTENT]\n"${content}"\n\n[OUTPUT]\nReturn ONLY raw JSON matching the schema: { title, essence, archetype, reasoning }`;
             response = await ai.models.generateContent({
                model,
                contents: gemmaPrompt,
                config: getGemmaConfig()
                ...
                config: {
                    ...
                    responseSchema: {
                        ...
                        properties: {
                            ...
                            archetype: { type: Type.STRING, enum: ['task', 'habit', 'insight'] },
                            reasoning: { type: Type.STRING },
                        },
                        required: ['title', 'essence', 'archetype', 'reasoning']
                    }
                }
             });
        }

        return parseJSON<HeroJourneyAnalysis>(response.text, { 
            title: "New Quest", 
            essence: "A call to adventure.", 
            archetype: 'task', 
            reasoning: "Default analysis." 
        });

    } catch (e) {
        console.error("Hero Journey Analysis Error", e);
        return null;
    }
};
